...
- WeChat DBs use WAL journal mode. We read with `READ_ONLY` + `busy_timeout=200ms` (short-lived connections). In WAL mode, readers and writers never block each other — no lock contention. `immutable=1` must NOT be used — it skips reading the WAL file and returns stale data. WeChat handles its own WAL checkpoints.
